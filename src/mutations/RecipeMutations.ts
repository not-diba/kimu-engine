      ...
      recipeCategories = {
        create: await Promise.all(
          args.categories.map(async (name) => {
            ...
            const category =
              existing ??
              (await ctx.prisma.category.create({ data: { name } }));
            ...
            RecipeIngredient: {
              create: await Promise.all(
                args.ingredients.map(async (ingredient) => {
                  ...
                  const normalizedInput = normalizeQuantity(
                    ingredient.quantity,
                    ingredient.unit,
                  );
                  const normalizedBase = normalizeQuantity(
                    ingredient.baseQuantity,
                    ingredient.defaultUnit,
                  );
                  const calculatedPrice =
                    (normalizedInput / normalizedBase) * ingredient.basePrice;
